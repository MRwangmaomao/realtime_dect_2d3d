...
// 1. 点云团分割，获取点云团================================= 
         ...
         indices_it != cluster_indices.end(); ++indices_it )
   {
	 ...
	 double mean_z =  sumz / seg->size(); //均值
         ...
         object3d.minPt    = Eigen::Vector4f(minmax_x.first->x, minmax_y.first->y, minmax_z.first->z,0.0);
         object3d.maxPt    = Eigen::Vector4f(minmax_x.second->x,minmax_y.second->y,minmax_z.second->z,0.0);
         object3d.centroid = Eigen::Vector4f(mean_x, mean_y, mean_z, 0.0); // 均值中心
         // 3d边框
